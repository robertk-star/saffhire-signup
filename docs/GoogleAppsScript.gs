 ...
 *   action: "upsert"  — find the row whose sessionId matches and update it in-place,
 *                        or append a new row if no match is found.
 *                        Used for real-time partial saves while the user is filling out the form.
 ...
 *   A: sessionId | B: status | C: submittedAt | D: companyName | E: ein
 *   F: businessEntity | G: ownerFirstName | H: ownerLastName | I: ownerEmail
 *   J: ownerPhone | K: ownerTitle | L: contactFirstName | M: contactLastName
 *   N: contactEmail | O: contactPhone | P: contactTitle | Q: businessStreet
 *   R: businessCity | S: businessState | T: businessZip | U: billingSameAsBusiness
 *   V: billingStreet | W: billingCity | X: billingState | Y: billingZip
 *   Z: adminUsers
 */
...
function upsertRow(sheet, sessionId, payload) {
  ...
  // Search for existing row (start from row index 1 to skip headers)
  ...
  if (existingRowIndex > 0) {
    // Update only the cells that have new values — preserve existing data
    for (var c = 0; c < COLUMNS.length; c++) {
      var colName = COLUMNS[c];
      var newValue = payload[colName];
      // Only overwrite if the incoming payload has a non-empty value for this field
...
function sendNotificationEmail(payload) {
  try {
    ...
    var body = [
      ...
      "",
      "Business Address:",
      (payload.businessStreet || "—") + ", " + (payload.businessCity || "") + ", " + (payload.businessState || "") + " " + (payload.businessZip || ""),
      "",
